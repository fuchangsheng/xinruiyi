...
class SysDataModel extends Model {
    ...
    public function getSqlFilesList() {
        $dataBackupPath = C('DATA_BACKUP_PATH');
        $list = array();
        $size = 0;
        $handle = opendir($dataBackupPath);

        while ($file = readdir($handle)) {
            if (preg_match('#\.sql$#i', $file)) {
                $dataBackupPath . "$file";
                $fp = fopen($dataBackupPath . "$file", 'rb');
                ...
                $_size = filesize($dataBackupPath . "/$file");
                ...
                $mtime = filemtime($dataBackupPath . "$file");
                $list[$mtime][$file] = $bk;
            }
        }
    ...
    public function getZipFilesList() {
        $list = array();
        $size = 0;
        $handle = opendir($this->dataBackupPath . "Zip/");

        while ($file = readdir($handle)) {
            if ($file != "." && $file != "..") {
                $tem = array();
                $tem['file'] = $file; //  checkCharset($file);
                $_size = filesize($this->dataBackupPath . "Zip/$file");
                $tem['size'] = byteFormat($_size);
                $tem['time'] = date("Y-m-d H:i:s", filectime($this->dataBackupPath . "Zip/$file"));
                $size+=$_size;
                $list[] = $tem;
            }
        }
    ...
    function unzip($file, $outDir = '') {
        $zip = new \ZipArchive();
        if ($zip->open($this->dataBackupPath . "Zip/" . $file) !== TRUE)
            return FALSE;
        $zip->extractTo($this->dataBackupPath);
        $zip->close();
        return TRUE;
    }
