...
class SysdataController extends CommonController {
   protected $dataBackupPath = '';
    function _initialize() {
        parent::_initialize();
        $this->dataBackupPath = C('DATA_BACKUP_PATH');
    }
    ...
    public function backup() {
        ...
        if(!is_dir($this->dataBackupPath)){
            mkdir($this->dataBackupPath);
        }

        $systemBackup = I('post.systemBackup');	//设置超级管理备份权限；
        ...
        if (count($tables) == 0 && empty($systemBackup)) {
            die(json_encode(array("status" => 0, "info" => "请先选择要备份的表")));
        }

        $time = time();
        if (!empty($systemBackup)) {
            ...
            $path = $this->dataBackupPath . "SYSTEM_" . date("Ym");
            if (file_exists($path . "_1.sql")) {
                die(json_encode(array("status" => 0, "info" => "本月度系统已经进行了自动备份操作")));
            }
        } else {
            $type = "管理员后台手动备份";
            $path = $this->dataBackupPath . "CUSTOM_" . date("Ymd") . "_" . date("His");
        }


        $pre = "# -----------------------------------------------------------\n" .
                "# LQBCMS database backup files\n" .
                "# Blog: http://www.liqingbo.cn\n" .
                "# Type: {$type}\n";
        ...
        foreach ($tables as $tableName) {
            $backedTable[] = $tableName;
            $outPut.="\n\n# 数据库表：{$tableName} 数据信息\n";
            $tableInfo = $M->query("SHOW TABLE STATUS LIKE '{$tableName}'");

            $page = ceil($tableInfo[0]['rows'] / 10000) - 1;	//


            for ($i = 0; $i <= $page; $i++) {
                $query = $M->query("SELECT * FROM {$tableName} LIMIT " . ($i * 10000) . ", 10000");
                ...
                foreach ($query as $val) {
                    ...
                    $temSql = "INSERT INTO `{$tableName}` VALUES ({$temSql});\n";
    ...
    static private function getRestoreFiles() {
        ...
        //获取sql文件前缀
        $sqlPre = $_GET['sqlPre'];
        $handle = opendir($this->dataBackupPath);
        $sqlFiles = array();
    ...
    public function restoreData() {
        ...
        foreach ($files as $fileKey => $sqlFile) {
            $file = $this->dataBackupPath . $sqlFile;
            if (!file_exists($file))
                continue;
            $file = fopen($file, "r");
            $sql = "";
            ...
            while (!feof($file)) {
                ...
                if ($tem[$end] == ";") {
                    ...
                    if ($execute > 500) {
                        ...
                        echo json_encode(array("status" => 1, "info" => '如果导入SQL文件卷较大(多)导入时间可能需要几分钟甚至更久，请耐心等待导入完成，导入期间请勿刷新本页，当前导入进度：<font color="red">已经导入' . $imported . '条Sql</font>', "url" => U('SysData/restoreData', array(get_randomstr() => get_randomstr()))));
                        exit;
                    }
                } else {
                    $sql.=$tem;
    ...
    public function delSqlFiles() {
        if (IS_POST) {
            ...
            foreach ($files as $file) {
                del_dir_and_file($this->dataBackupPath . $file);
            }
            echo json_encode(array("status" => 1, "info" => "已删除：" . implode("、", $files), "url" => __CONTROLLER__ . "/restore?" . time()));
        }
    }
...
//发送邮件
                    ...
                    del_dir_and_file(WEB_CACHE_PATH . $zipOut); //删除已发送附件
                    echo json_encode(array("status" => 1, "info" => "如果要发送SQL文件卷较大(多)发送时间可能需要几分钟甚至更久，请耐心等待，发送期间请勿刷新本页。SQL打包成{$sum}个zip包，分{$sum}封邮件发出，<font color=\"red\">当前已经发送完第{$k}封邮件</font>", "url" => U('SysData/sendSql', array(get_randomstr() => get_randomstr()))));
    ...
    public function zipSql() {
        if (IS_POST) {
            ...
            foreach ($toZip as $key => $val) {
                if (D("SysData")->zip($val, $key . ".zip", $this->dataBackupPath . "Zip/",$this->dataBackupPath)) {
                    foreach ($val as $file) {
                        del_dir_and_file($this->dataBackupPath . $file);
                    }
                }
    ...
    function unzipSqlfile() {
        ...
        foreach ($files as $k => $file) {
            ...
            if (count($files) > 1) {
                echo json_encode(array("status" => 1, "info" => "正在解压缩请耐心等待，解压期间请勿刷新本页 <font color=\"red\">当前已经解压完{$file}</font>", "url" => U('SysData/unzipSqlfile', array(get_randomstr() => get_randomstr()))));
    ...
    public function delZipFiles() {
        if (IS_POST) {
            ...
            $files = $_POST['zipFiles'];
            foreach ($files as $file) {
                del_dir_and_file($this->dataBackupPath . "Zip/" . $file);
            }
    ...
    public function downFile() {
        ...
        $path = array("zip" => $this->dataBackupPath . "Zip/", "sql" => $this->dataBackupPath);
    ...
    public function fields(){
        ...
        //$M->query("CREATE TABLE lqb_test3 LIKE lqb_test");
        $a = $M->query("show create database test");
        print_r($a);
